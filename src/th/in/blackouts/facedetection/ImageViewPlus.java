...
class ImageViewPlus extends ImageView {
	...
	public ImageViewPlus(Context c) {
		super(c);         
		init();               
	}             
	public ImageViewPlus(Context c, AttributeSet attrs) {
		super(c, attrs);
		init();
	}
	private void init() {                
			...
			if (mPX != null && mPY != null) {
				for (int i = 0; i < mPX.length; i++) {
					if (mDisplayStyle == 1) {
						...
						canvas.drawRect(mPX[i] - 60,  mPY[i] - 60, mPX[i] + 60,  mPY[i] + 60, mPaint);
					}
				}
			}
		}
